...
class CreateNewGame(Scene):
	...
	def choose_save(self):
		...
		for letter in self.text_list:
			savestring += letter
		self.game.world.player.savename = savestring
		...
		for index, letter in enumerate(self.text_list):
			if index + 1 <= len(self.name_length):
				draw_custom_font_text(
					...
					letter,
					96,
					self.letter_slots[index].topleft,
					[' ', '.']
				)			
			else:
...
class ChooseSave(Scene):
	...
	def callback(self, slot_name):
		if self.pick_type in ['new game']:
			save_game(self.game.world, self.game.world.player, slot_name)
			self.game.world.player.saveslot = slot_name
			...
			if slot_name == 'No data':
				...
				load_save(self.game.world, self.game.world.player, slot_name)
				self.game.world.player.saveslot = slot_name
	...
	def draw(self):
		...
		draw_custom_font_text(self.game.screen, self.game.font, self.game.world.player.savename, 128, (200, self.game.screen.get_rect().top + 20), [])
...
class World(Scene):
	...
	def __init__(self, game, world_data):
		...
		# create camera
		self.camera = Camera(self.game, 12, 250)

		# player setup
		self.player_spawn = self.generate_player_spawn(self.world_data)
		self.player_sprite_group = pygame.sprite.GroupSingle()
		self.player = Player(self.game, "ALRYN", CHARACTERS["ALRYN"]["SPRITE SIZE"], self.player_spawn, CHARACTERS["ALRYN"]["SPEED"], [self.player_sprite_group])
		...
		self.full_background = scale_images(self.full_background, (self.level_width, self.level_height))
		...
		# music
		pygame.mixer.music.load(f'../assets/music/{self.game.current_world}/{WORLDS[self.game.current_world]["music"][0]}.wav')
	...
	def world_FX(self):
		...
		for spell in self.game.world.player.projectiles:
			if spell.status not in ['hit', 'remove']:
				...
				self.world_brightness.blit(spell_glow, spell.rect.topleft - self.camera.level_scroll - (30, 30), special_flags=BLEND_RGB_ADD)
